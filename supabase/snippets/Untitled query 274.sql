...
BEGIN
  ...
  -----------------------------------------------------------------------------
  -- B. ÉQUIPEMENTS ENERGÉTIQUES (ASSETS)
  -----------------------------------------------------------------------------
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'app' AND table_name = 'energy_assets') THEN
    ...
    VALUES 
      (v_asset_ep1_id, v_org_main_id, 'Armoire EP - Centre Ville', 'Eclairage Public'),
      (v_asset_ep2_id, v_org_main_id, 'Poste HT/BT - Zone Industrielle', 'Eclairage Public'),
      (v_asset_irve1_id, v_org_main_id, 'Station IRVE - Place du Marché', 'IRVE'),
      (v_asset_irve2_id, v_org_main_id, 'Borne Rapide 50kW - Gare', 'IRVE'),
      (v_asset_pv1_id, v_org_main_id, 'Centrale Solaires - Gymnase Municipal', 'Photovoltaïque')
  ...
  -- --------------------------------------------------------------------------
  -- 5. CAS NEUTRE (0 Ticket) : Valérie (DGS), Alexandre (DST), Thomas (CA), Jean-Pierre (Élu)
  -- --------------------------------------------------------------------------

  -- --------------------------------------------------------------------------
  -- 6. TICKETS EN ATTENTE D'ASSIGNATION (Backlog général)
  -- --------------------------------------------------------------------------
  INSERT INTO app.tickets_maintenance (organization_id, asset_id, title, description, status, priority, assigned_contact_id)
  VALUES 
