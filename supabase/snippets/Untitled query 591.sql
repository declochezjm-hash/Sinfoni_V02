...
BEGIN
    ...
    -- 3. Injection des chantiers de démo avec les statuts adaptés à la contrainte CHECK
    IF NOT EXISTS (SELECT 1 FROM app.projects) THEN
        ...
        VALUES 
            (gen_random_uuid(), 'PRJ-2026-001', 'eclairage_public', 'Rénovation Éclairage Public - Route de Banyuls', 'En cours', v_org_id, NOW()),
            (gen_random_uuid(), 'PRJ-2026-002', 'assainissement', 'Inspection Télévisée Réseau EU - Centre Bourg', 'Brouillon', v_org_id, NOW()),
            (gen_random_uuid(), 'PRJ-2026-003', 'eau_potable', 'Mise en conformité AEP - Secteur Nord', 'Terminé', v_org_id, NOW());
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM app.tickets_maintenance) THEN
        ...
        VALUES 
            (gen_random_uuid(), 'Remplacement lanterneau défectueux', 'Nouveau', 'Haute', v_org_id, NOW()),
            (gen_random_uuid(), 'Réparation fuite sous chaussée (AEP)', 'En cours', 'Urgente', v_org_id, NOW()),
            (gen_random_uuid(), 'Entretien préventif poste de refoulement', 'Fermé', 'Normale', v_org_id, NOW());
    END IF;
END $$;
