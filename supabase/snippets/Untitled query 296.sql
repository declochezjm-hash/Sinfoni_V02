...
BEGIN
    ...
    -- 3. Si la table des projets est complètement vide, injecter 3 chantiers de démo
    IF NOT EXISTS (SELECT 1 FROM app.projects) THEN
        INSERT INTO app.projects (id, title, status, organization_id, created_at)
        VALUES 
    ...
    -- 4. Si la table des interventions est complètement vide, injecter 3 tickets de démo
