...
BEGIN
  ...
  -----------------------------------------------------------------------------
  -- D. TICKETS DE MAINTENANCE (avec commune_insee_code obligatoire)
  -----------------------------------------------------------------------------
  
  -- 1. SURCHARGE (5 Tickets) -> Lucas Moreau (Badge Rouge 🔴)
  INSERT INTO app.tickets_maintenance (organization_id, asset_id, commune_insee_code, title, description, status, priority, assigned_contact_id)
  VALUES 
    (v_org_main_id, v_asset_ep1_id, '69123', 'Panne secteur Allée des Lices', 'Coupure totale sur 18 candélabres.', 'open', 'urgent', v_tech1_id),
    (v_org_main_id, v_asset_ep1_id, '69123', 'Contrôle réglementaire armoire N°4', 'Visite annuelle de conformité électrique.', 'in_progress', 'low', v_tech1_id),
    (v_org_main_id, v_asset_ep2_id, '69123', 'Horloge astronomique hs', 'Éclairage reste allumé en journée.', 'open', 'high', v_tech1_id),
    (v_org_main_id, v_asset_ep2_id, '69123', 'Remplacement mât endommagé', 'Choc véhicule léger sur mât N°12.', 'open', 'high', v_tech1_id),
    (v_org_main_id, v_asset_ep1_id, '69123', 'Reprise étanchéité boîte de jonction', 'Infiltration eau observée lors du dernier passage.', 'open', 'medium', v_tech1_id);

  -- 2. NORMAL (2 Tickets) -> Sophie Lefebvre (Badge Vert 🟢)
  INSERT INTO app.tickets_maintenance (organization_id, asset_id, commune_insee_code, title, description, status, priority, assigned_contact_id)
  VALUES 
    (v_org_main_id, v_asset_irve1_id, '69266', 'Défaut de communication 4G', 'La borne n''envoie plus les télémesures.', 'in_progress', 'high', v_tech2_id),
    (v_org_main_id, v_asset_irve2_id, '69266', 'Verrouillage prise T2 défaillant', 'Bloque les usagers en fin de charge.', 'open', 'medium', v_tech2_id);

  -- 3. NORMAL (1 Ticket) -> Camille Rousseau (Badge Vert 🟢)
  INSERT INTO app.tickets_maintenance (organization_id, asset_id, commune_insee_code, title, description, status, priority, assigned_contact_id)
  VALUES 
    (v_org_main_id, v_asset_pv1_id, '69029', 'Validation étude raccordement PV', 'Analyse du dossier d''injection réseau.', 'in_progress', 'low', v_ca1_id);

  -- 4. NORMAL (3 Tickets) -> Maxime Gautier (Badge Vert 🟢)
  INSERT INTO app.tickets_maintenance (organization_id, asset_id, commune_insee_code, title, description, status, priority, assigned_contact_id)
  VALUES 
    (v_org_main_id, v_asset_ep1_id, '69123', 'Nettoyage des crosses et verres', 'Entretien préventif trimestriel.', 'open', 'low', v_tech3_id),
    (v_org_main_id, v_asset_ep2_id, '69123', 'Test d''isolement ligne Sud', 'Mesure préventive d''isolement des câbles.', 'open', 'medium', v_tech3_id),
    (v_org_main_id, v_asset_ep1_id, '69123', 'Remplacement cellule photoélectrique', 'Éclairage clignote au crépuscule.', 'in_progress', 'medium', v_tech3_id);

  -- 5. TICKETS EN ATTENTE D'ASSIGNATION
  INSERT INTO app.tickets_maintenance (organization_id, asset_id, commune_insee_code, title, description, status, priority, assigned_contact_id)
  VALUES 
    (v_org_main_id, v_asset_irve2_id, '69266', 'Demande d''extension station IRVE', 'Ajout de 2 points de charge supplémentaires.', 'open', 'medium', NULL),
    (v_org_main_id, v_asset_pv1_id, '69029', 'Nettoyage des panneaux solaires', 'Baisse de rendement observée (-12%).', 'open', 'low', NULL);
