...
BEGIN
  ...
  -----------------------------------------------------------------------------
  -- 3. INSERTION DES ÉQUIPEMENTS (avec latitude / longitude)
  -----------------------------------------------------------------------------
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'app' AND table_name = 'energy_assets') THEN
    INSERT INTO app.energy_assets (id, organization_id, name, type, status, commune_insee_code, latitude, longitude)
    VALUES 
    (v_asset_irve1_id, v_org_id, 'Station IRVE - Place du Marché', 'IRVE', 'active', '69123', 45.7675, 4.8562),
    (v_asset_irve2_id, v_org_id, 'Borne de recharge Rapide - Mairie', 'IRVE', 'maintenance', '69266', 45.7667, 4.8792),
    (v_asset_ep1_id, v_org_id, 'Armoire EP - Avenue Jean Jaurès', 'Eclairage Public', 'active', '69029', 45.7333, 4.9117),
    (v_asset_ep2_id, v_org_id, 'Poste de Transformation EP - Centre', 'Eclairage Public', 'active', '69142', 45.7667, 4.9583)
    ON CONFLICT DO NOTHING;
  END IF;

  -----------------------------------------------------------------------------
  -- 4. INSERTION DES TICKETS DE MAINTENANCE
  -----------------------------------------------------------------------------
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'app' AND table_name = 'tickets_maintenance') THEN
    ...
    -- Tickets 2, 3, 4, 5 : Assignés à Lucas Moreau (Charge = 4 -> Badge Rouge 🔴)
    INSERT INTO app.tickets_maintenance (organization_id, title, description, status, priority, asset_id, assigned_contact_id)
    VALUES 
