...
BEGIN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'app' AND table_name = 'tickets_maintenance') THEN
    ...
    INSERT INTO app.tickets_maintenance (organization_id, title, description, status, priority, assigned_contact_id)
    VALUES 
    (v_org_id, 'Remplacement lanterne LED', 'Horloge astronomique déréglée.', 'open', 'medium', v_tech1_id),
    (v_org_id, 'Contrôle annuel des armoires', 'Visite réglementaire de sécurité.', 'in_progress', 'low', v_tech1_id),
    (v_org_id, 'Câble détérioré suite travaux', 'Poteau endommagé par un engin de chantier.', 'open', 'urgent', v_tech1_id),
    (v_org_id, 'Nettoyage des optiques', 'Maintenance préventive programmée.', 'open', 'low', v_tech1_id);

    -- Ticket 6 : Assigné à Sophie Lefebvre (Charge = 1 -> Badge Vert 🟢)
    INSERT INTO app.tickets_maintenance (organization_id, title, description, status, priority, assigned_contact_id)
    VALUES (
      v_org_id, 
