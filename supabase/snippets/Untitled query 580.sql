...
BEGIN
    ...
    -- 3. Injection des chantiers de démo avec reference et type renseignés
    IF NOT EXISTS (SELECT 1 FROM app.projects) THEN
        ...
        VALUES 
            (gen_random_uuid(), 'PRJ-2026-001', 'eclairage_public', 'Rénovation Éclairage Public - Route de Banyuls', 'in_progress', v_org_id, NOW()),
            (gen_random_uuid(), 'PRJ-2026-002', 'assainissement', 'Inspection Télévisée Réseau EU - Centre Bourg', 'planned', v_org_id, NOW()),
            (gen_random_uuid(), 'PRJ-2026-003', 'eau_potable', 'Mise en conformité AEP - Secteur Nord', 'completed', v_org_id, NOW());
    END IF;
