...
BEGIN
  ...
  -----------------------------------------------------------------------------
  -- 3. INSERTION DES ÉQUIPEMENTS (Insertion générique sans colonne d'emplacement)
  -----------------------------------------------------------------------------
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'app' AND table_name = 'energy_assets') THEN
    INSERT INTO app.energy_assets (id, organization_id, name, type, status)
    VALUES 
    (v_asset_irve1_id, v_org_id, 'Station IRVE - Place du Marché', 'IRVE', 'active'),
    (v_asset_irve2_id, v_org_id, 'Borne de recharge Rapide - Mairie', 'IRVE', 'maintenance'),
    (v_asset_ep1_id, v_org_id, 'Armoire EP - Avenue Jean Jaurès', 'Eclairage Public', 'active'),
    (v_asset_ep2_id, v_org_id, 'Poste de Transformation EP - Centre', 'Eclairage Public', 'active')
