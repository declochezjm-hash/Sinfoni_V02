...
BEGIN
  ...
  -----------------------------------------------------------------------------
  -- 3. INSERTION DES ÉQUIPEMENTS (avec commune_insee_code obligatoire)
  -----------------------------------------------------------------------------
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'app' AND table_name = 'energy_assets') THEN
    INSERT INTO app.energy_assets (id, organization_id, name, type, status, commune_insee_code)
    VALUES 
    (v_asset_irve1_id, v_org_id, 'Station IRVE - Place du Marché', 'IRVE', 'active', '69123'),
    (v_asset_irve2_id, v_org_id, 'Borne de recharge Rapide - Mairie', 'IRVE', 'maintenance', '69266'),
    (v_asset_ep1_id, v_org_id, 'Armoire EP - Avenue Jean Jaurès', 'Eclairage Public', 'active', '69029'),
    (v_asset_ep2_id, v_org_id, 'Poste de Transformation EP - Centre', 'Eclairage Public', 'active', '69142')
