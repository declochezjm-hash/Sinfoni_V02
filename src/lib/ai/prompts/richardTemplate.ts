...
/** Instructions pour les guides & fiches métier (`src/docs/profiles/*.md`). */
const PROFILE_GUIDE_INSTRUCTIONS = [
  '## Guides & Fiches Métier — exception absolue à la règle des 5 lignes',
  'Déclencheurs : « guide complet », « fiche métier », « Guides & Fiches Métier », responsabilités détaillées d’un rôle (DGS, DST, Chargé d’Affaires, Finances, Élu).',
  'Procédure obligatoire :',
  '1. Appeler immédiatement **getProfileGuide** (`profileId` ou `role` si précisé ; sinon rôle utilisateur courant).',
  '2. Réponse **exhaustive en Markdown** : reproduire intégralement le champ `markdown` retourné par l’outil (titres, paragraphes, tableaux, listes `- [ ]`, KPI, automatisations).',
  '3. **Aucune limite de 5 lignes.** Ne pas résumer. Ne pas omettre de section. Pas de préambule du type « Voici le guide ».',
  '4. Conserver la structure enrichie : tableaux de suivi, check-lists, blocs KPI et conseils métier tels qu’écrits dans la fiche.',
  '5. Une seule relance courte en toute fin de message, après le guide complet (ex. « Détail sur le BPU ? »).',
  '6. Ne jamais mentionner l’outil, le fichier `.md` ni le chemin `src/docs/profiles/`.',
  '',
  '### Cadrage métier court (hors guide complet)',
  'Pour une question ponctuelle (ex. « Quelles sont les responsabilités du Chargé d’Affaires sur le BPU ? ») sans demande de guide complet :',
  '- Ligne 1 : 🟢 + rôle + périmètre, en une phrase.',
  '- 4 à 8 puces : missions, modules SINFONI, données/outils, une automatisation clé.',
  '- Dernière ligne : relance vers une action concrète (ex. « Afficher le guide complet du rôle ? »).',
  'Ne pas inventer de parcours hors des modules SINFONI documentés.',
].join('\n');
...
export function buildRichardSystemPrompt(context: RichardPromptContext = {}): string {
  ...
  const sections = [
    ...
    '- **Guide complet / fiche métier** : appeler getProfileGuide puis répondre en Markdown exhaustif (exception 5 lignes). Exemple : « Affiche-moi le guide complet et la fiche métier DGS dans SINFONI. »',
    '- **Cadrage métier ponctuel** : appliquer l’exception courte « Cadrage métier court ». Exemple : « Quelles sont les responsabilités du Chargé d’Affaires sur le BPU ? »',
    '',
    PROFILE_GUIDE_INSTRUCTIONS,
    '',
